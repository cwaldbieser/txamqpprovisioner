    ...
    * unmanaged_id: Some remote accounts are specific to the remote service and
    ...
    def parse_config(self, scp):
        ...
        if api_username is None:
            raise OptionMissingError("The `api_username` option is missing!")
    ...
    def api_get_auth_token(self):
        ...
        response = yield http_client.get(
            auth_url, auth=basic_auth, params=params, headers=headers
        )
        ...
        if resp_code == 200:
            ...
            if not "data" in doc:
                ...
                raise Exception(
                    "Error parsing authentication response.  Missing element `data`."
                )
            data = doc["data"]
            if not "v3_user_token" in data:
                ...
                raise Exception(
                    "Error parsing authentication response.  Missing element `v3_user_token`."
                )
    ...
    def get_all_api_ids_and_match_values(self):
        ...
        while True:
            ...
            if data is None:
                ...
                raise Exception(
                    "Error attempting to parse response to get ALL users.  Missing `data` element."
                )
            ...
            if users is None:
                log.error("Error attempting to parse response to get ALL users.")
                raise Exception(
                    "Error attempting to parse response to get ALL users.  Missing `users` element."
                )
            for entry in users:
    ...
    def change_subject_status_(self, api_id, active=True):
        ...
        log.debug("headers: {headers}", headers=headers)
        resp = yield self.make_authenticated_api_call(method, url, headers=headers)
        resp_code = resp.code
        try:
    ...
    def api_get_account_id(self, subject, attributes):
        ...
        if resp_code not in (200,):
            raise Exception(
                "API call to fetch remote account ID returned HTTP status {}".format(
                    resp_code
                )
            )
        data = parsed.get("data", None)
    ...
    def api_update_subject(self, subject, api_id, attributes):
        ...
        if resp_code != 204:
            raise Exception(
                "API call to activate subject returned HTTP status {}".format(resp_code)
            )
        prefix = self.url_prefix
        url = "{}/api/User/{}".format(prefix, api_id)
        headers = {
    ...
    def api_add_subject(self, subject, attributes):
        ...
        if api_id:
            ...
            if resp_code != 204:
                raise Exception(
                    "API call to activate subject returned HTTP status {}".format(
                        resp_code
                    )
                )
            returnValue(api_id)
