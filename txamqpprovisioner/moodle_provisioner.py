...
from rest_provisioner import RESTProvisioner, RESTProvisionerFactory
    ...
    * unmanaged_id: Some remote accounts are specific to the remote service and
    ...
    def check_for_error_response(self, json_doc):
        ...
        if "errorcode" in json_doc:
            raise Exception(
                "{0} - {1}".format(json_doc["errorcode"], json_doc["message"])
            )
    ...
    def api_add_subject(self, subject, attributes):
        ...
        params = {"moodlewsrestformat": "json"}
        first_name = (attributes.get("displayName", [""])[0]) or (
        ...
        data = {
            ...
            "users[0][customfields][0][value]": attributes.get("bannerLNumber", [""])[
                0
            ],
